...
Приложение построено по клиент-серверной архитектуре. Frontend представляет собой единое React SPA, которое может работать в двух режимах:

- **Telegram Mini App** -- запускается внутри Telegram-клиента (WebView / iframe), использует Telegram Web App SDK для интеграции с платформой (тема, навигация, данные пользователя).
- **Standalone SPA** -- открывается напрямую в браузере как обычное веб-приложение, без привязки к Telegram.
